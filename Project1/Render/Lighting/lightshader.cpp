...
bool LIGHTSHADER::InitializeShaderBuffer()
{
	HRESULT result;
...
bool LIGHTSHADER::Render(int indexCount, RNDMATRIXS matrixs, D3DXVECTOR3 cameraPosition)
{
	return Render(indexCount, matrixs, cameraPosition, nullptr);
}

bool LIGHTSHADER::Render(int indexCount, RNDMATRIXS matrixs, D3DXVECTOR3 cameraPosition, ID3D11ShaderResourceView* texture)
{
	render = matrixs;
	ISFAIL(SetShaderParameters(cameraPosition, texture));
	RenderShader(indexCount);

	return true;
}
bool LIGHTSHADER::SetShaderParameters(D3DXVECTOR3 cameraPosition, ID3D11ShaderResourceView* texture)
{
	HRESULT result;
