...
{
	return InitializeShader(const_cast<WCHAR*>(L"./data/light.vs"), const_cast<WCHAR*>(L"./data/light.ps"));
}
...
bool LIGHTSHADER::SetShaderParameters(D3DXVECTOR3 cameraPosition, ID3D11ShaderResourceView* texture, LightBufferType lightType)
{
	...
	dataPtr2->ambientColor = lightType.ambientColor;
	dataPtr2->diffuseColor = lightType.diffuseColor;
	dataPtr2->lightDirection = lightType.lightDirection;
	dataPtr2->specularColor = lightType.specularColor;
	dataPtr2->specularPower = lightType.specularPower;
